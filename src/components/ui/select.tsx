...
import type { CollectionItem, RecipeVariantProps, SelectRootProps } from '@chakra-ui/react';
import { Select as ChakraSelect, Portal, useRecipe } from '@chakra-ui/react';
import { CloseButton } from './close-button';
import * as React from 'react';
import { SelectRecipe } from '@/components/styling/select.recipe';
...
export interface SelectProps extends SelectRootProps, SelectVariantProps {}

export const SelectRoot = React.forwardRef<HTMLDivElement, SelectProps>(function SelectRoot(props, ref) {
  const { visual, children, ...restProps } = props;
  const recipe = useRecipe({ key: "select" });
  const styles = recipe({ visual });
  return (
    <ChakraSelect.Root css={styles} {...restProps} ref={ref} positioning={{ sameWidth: true, ...props.positioning }}>
      {props.asChild ? (
        props.children
      ) : (
        <>
